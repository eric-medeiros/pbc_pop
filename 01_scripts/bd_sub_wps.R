...
bd_sub_wps <- function(dados_excel, dados_wps) {
  ...
  dados_wps_tudo <- dados_excel$wps_extra %>%
  ...
  atualiza_coordenadas_generico <- function(dados, wp_sub, tipo) {
    dados[[tipo]] <- dados[[tipo]] %>%
      ...
      rename(wp_I = wp_i) %>%
      dplyr::select(1:wp_I, datahora_I = datahora, lng_I = lng, lat_I = lat, wp_I:last_col()) %>%
      left_join(wp_sub, by = c("saida", "wp_f" = "wp")) %>%
      rename(wp_F = wp_f) %>%
      dplyr::select(1:wp_F, datahora_F = datahora, lng_F = lng, lat_F = lat, wp_I:last_col()) %>%
  ...
  dados_excel$caminhos <- dados_wps_tudo %>%
    select(-lng, -lat, -datahora) %>%
    filter(arquivo_wp != "NA") %>%
    nest(id = c(saida, wp)) %>%
    transmute(tipo = "gps_wp",
              id = id,
